...
#[derive(Clone)]
pub struct LumpCache(Option<Arc<[u8]>>);
impl LumpCache {
    //TODO MV: make this private
    ...
    pub(crate) fn as_ptr_mut(&self) -> *mut u8 {
        {
            self.0.as_ref().unwrap().as_ptr() as *mut u8
        }
    }
...
thread_local! {
    static WAD_STATE: RefCell<WadState> = const { RefCell::new(WadState {
        lumpinfo: Vec::new(),
        lumphash: None,
...
fn lump_name_matches(lump_name: &[u8; 8], name: &str) -> bool {
    ...
    for i in 0..8 {
        ...
        let a_up = if a.is_ascii_lowercase() { a - 32 } else { a };
        let b_up = if b.is_ascii_lowercase() { b - 32 } else { b };
        if a_up != b_up {
            return false;
        }
        if a == 0 {
...
pub fn w_add_file(filename: &str) -> Option<usize> {
    ...
    wad_state_write(|state| {
            ...
            && filename[filename.len() - 3..].eq_ignore_ascii_case("wad")
        {
            ...
            let wad = &state.open_wad_files[wad_index]; // this is the new wad_file
            ...
            let mut fileinfo = vec![0u8; length];
            let n = wad.read(infotableofs as u32, &mut fileinfo);
            if n < length {
                return None;
            }
...
pub fn w_read_lump(lump: usize, dest: &mut [u8]) {
    let (wad_index, position, size) = wad_state_read(|state| {
        ...
        (
            lump_info.wad_file_index,
            lump_info.position,
            lump_info.size as usize,
        )
    });
...
pub fn w_cache_lump_num(lumpnum: i32, tag: i32) -> LumpCache {
    ...
    let cache_opt = wad_state_read(|state| state.lumpinfo[lumpnum as usize].cache.clone());
    ...
    let n = wad_state_read(|state| {
        let wad = &state.open_wad_files[wad_index];
        wad.read(position as u32, &mut cache)
    });
    i_system::i_end_read();
    if n < size {
        i_system::i_error(&format!(
    ...
    let cache = LumpCache(Some(cache.into()));
    wad_state_write(|state| {
        state.lumpinfo[lumpnum as usize].cache = Some(cache.clone());
    });
    cache
}

/// Original: W_CacheLumpName
pub fn w_cache_lump_name(name: &str, tag: i32) -> LumpCache {
    w_cache_lump_num(w_get_num_for_name(name), tag)
}

/// Original: W_ReleaseLumpNum
pub fn w_release_lump_num(_lumpnum: i32) {}
