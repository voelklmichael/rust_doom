...
/// Lookup table for deterministic random numbers (0-255).
...
static RNDINDEX: AtomicU8 = AtomicU8::new(0);
static PRNDINDEX: AtomicU8 = AtomicU8::new(0);

/// Returns a number from 0 to 255, from a lookup table.
#[inline]
pub fn m_random() -> i32 {
    let idx = RNDINDEX.fetch_add(1, Ordering::SeqCst).wrapping_add(1);
    RNDTABLE[idx as usize] as i32
}

/// As M_Random, but used only by the play simulation.
#[inline]
pub fn p_random() -> i32 {
    let idx = PRNDINDEX.fetch_add(1, Ordering::SeqCst).wrapping_add(1);
    RNDTABLE[idx as usize] as i32
}
...
pub fn m_clear_random() {
    RNDINDEX.store(0, Ordering::SeqCst);
    PRNDINDEX.store(0, Ordering::SeqCst);
}
