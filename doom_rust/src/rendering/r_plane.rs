...
impl PlaneState {
    pub fn r_find_plane(&mut self, height: Fixed, picnum: i32, lightlevel: i32) -> &mut Visplane {
        ...
        for (check_index_, check) in self.visplanes.iter().enumerate() {
            if (*check).height == height
                && (*check).picnum == picnum
                && (*check).lightlevel == lightlevel
            {
                check_index = Some(check_index_);
                break;
            }
        }
        if let Some(check_index) = check_index {
            return &mut self.visplanes[check_index];
        } else {
            self.visplanes.push(Visplane::default());
            let check = self.visplanes.last_mut().unwrap();
            check.height = height;
            check.picnum = picnum;
            check.lightlevel = lightlevel;
            check.minx = SCREENWIDTH;
            check.maxx = -1;
            check.top = [PLANE_SENTINEL; _];
    ...
    pub fn r_init_planes(&mut self) {}

    /// Clear planes at beginning of frame.
    pub fn r_clear_planes(&mut self) {
        ...
        let (centerxfrac, projection, centery) =
            crate::rendering::r_main::with_r_main_state(|rm| {
                (rm.centerxfrac, rm.projection, rm.centery)
            });
        ...
        let (_d, ds_xstep, ds_ystep, _l, ds_xfrac, ds_yfrac, ds_colormap) =
            crate::rendering::r_draw::with_r_draw_state(|rd| {
                let (distance, ds_xstep, ds_ystep) = if ps.planeheight != ps.cachedheight[y] {
                    ...
                    (ps.cacheddistance[y], ps.cachedxstep[y], ps.cachedystep[y])
                };
    ...
    pub fn r_draw_planes() {
        with_plane_state(|ps| {
            let (skyflatnum, detailshift, extralight) =
                crate::rendering::r_main::with_r_main_state(|rm| {
                    (
                        crate::rendering::r_sky::with_r_sky_state(|rs| rs.skyflatnum),
                        rm.detailshift,
                        rm.extralight,
                    )
                });
            ...
            while pl_idx < last_idx {
                ...
                let mut light = unsafe { (*pl).lightlevel >> LIGHTSEGSHIFT } as i32 + extralight;
                ...
                ps.planezlight = crate::rendering::r_main::with_r_main_state(|rm| {
                    rm.zlight[light as usize].as_ptr() as *mut *mut u8
                });
